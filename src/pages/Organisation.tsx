...
import { Plus, Zap, Brain, Clock, CheckCircle2, Circle, ArrowUpRight, Trash2, ChevronDown, ChevronRight, CalendarDays, LayoutGrid, FolderKanban, Layers } from "lucide-react";
  ...
  return (
    <div className="flex h-screen flex-col overflow-auto p-8">
      ...
      <header className="mb-8">
        <h1 className="text-4xl font-black text-foreground tracking-tight">Organisation</h1>
        <p className="text-muted-foreground font-medium mt-1">Gérez vos espaces de travail et vos priorités.</p>
      </header>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogTrigger asChild>
            <button className="flex items-center justify-center gap-2 h-14 rounded-2xl bg-primary text-primary-foreground font-bold shadow-lg shadow-primary/10 hover:shadow-primary/20 transition-all">
              <Plus size={18} /> Tâche complète
            </button>
          </DialogTrigger>
          <DialogContent className="bg-card border-border">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-4">
                ...
                <label className="flex items-center gap-2 text-sm"><Checkbox checked={newTaskNeedle} onCheckedChange={(v) => setNewTaskNeedle(!!v)} /> Move the Needle</label>
              </div>
              <Button className="w-full bg-primary text-primary-foreground" onClick={handleAddTask}>Ajouter</Button>
            </div>
          </DialogContent>
        </Dialog>
        <button className="flex items-center justify-center gap-2 h-14 rounded-2xl bg-card text-foreground font-bold shadow-sm hover:shadow-md transition-all">
          <Zap size={18} className="text-primary" /> Tâche rapide
        </button>
        <button className="flex items-center justify-center gap-2 h-14 rounded-2xl bg-card text-foreground font-bold shadow-sm hover:shadow-md transition-all">
          <Clock size={18} className="text-primary" /> Do Now
        </button>
        <button className="flex items-center justify-center gap-2 h-14 rounded-2xl bg-card text-foreground font-bold shadow-sm hover:shadow-md transition-all">
          <Brain size={18} className="text-primary" /> Routine
        </button>
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="bg-card rounded-2xl shadow-sm overflow-hidden">
          <button onClick={() => toggle("today")} className="flex items-center justify-between p-6 w-full hover:bg-secondary/50 transition-colors">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-lg font-bold tracking-wide text-foreground uppercase">Aujourd'hui</h3>
            </div>
            <span className="text-sm font-medium text-muted-foreground">{remainingCount} tâches restantes</span>
          </button>
          {openSections.today && (
              ...
              <div className="grid grid-cols-12 gap-8">
                <div className="col-span-8 flex flex-col gap-3">
                  {todayTasks.map(t => {
                    ...
                    return (
                      <div key={t.id} className="flex items-center justify-between bg-card p-4 rounded-xl shadow-sm">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className={`font-semibold ${t.done ? "text-muted-foreground line-through" : "text-foreground"}`}>{t.text}</p>
                ...
                {/* Mini Calendar */}
                <div className="col-span-4 bg-card p-5 rounded-xl shadow-sm border border-border">
        ...
        {/* CALENDRIER */}
        <div className="bg-card rounded-2xl shadow-sm overflow-hidden">
          <button onClick={() => toggle("calendar")} className="flex items-center justify-between p-6 w-full hover:bg-secondary/50 transition-colors">
            <div className="flex items-center gap-3">
              {openSections.calendar ? <ChevronDown size={20} className="text-primary" /> : <ChevronRight size={20} className="text-muted-foreground" />}
              <h3 className="text-lg font-bold tracking-wide text-foreground uppercase">Calendrier</h3>
            </div>
          ...
          {openSections.calendar && (
            <div className="p-6 border-t border-border bg-secondary/20">
              <div className="grid grid-cols-7 gap-2">
                {Array.from({ length: 7 }, (_, i) => (
                  <div key={i} className="h-32 rounded-xl bg-card shadow-sm border border-border" />
                ))}
              </div>
            </div>
          )}
        ...
        {/* TÂCHES (Eisenhower) */}
        <div className="bg-card rounded-2xl shadow-sm overflow-hidden">
          <button onClick={() => toggle("tasks")} className="flex items-center justify-between p-6 w-full hover:bg-secondary/50 transition-colors">
            <div className="flex items-center gap-3">
              {openSections.tasks ? <ChevronDown size={20} className="text-primary" /> : <ChevronRight size={20} className="text-muted-foreground" />}
              <h3 className="text-lg font-bold tracking-wide text-foreground uppercase">Tâches</h3>
            </div>
            <span className="text-xs font-bold bg-primary/10 text-primary px-3 py-1 rounded-full">Matrice Eisenhower</span>
          </button>
          {openSections.tasks && (
            <div className="p-6 border-t border-border bg-secondary/20">
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={q.label} className={`p-4 rounded-xl border ${q.cls} min-h-[120px]`}>
        ...
        {/* PROJETS */}
        <div className="bg-card rounded-2xl shadow-sm overflow-hidden">
          <button onClick={() => toggle("projects")} className="flex items-center justify-between p-6 w-full hover:bg-secondary/50 transition-colors">
            <div className="flex items-center gap-3">
              {openSections.projects ? <ChevronDown size={20} className="text-primary" /> : <ChevronRight size={20} className="text-muted-foreground" />}
              <h3 className="text-lg font-bold tracking-wide text-foreground uppercase">Projets</h3>
            </div>
          ...
          {openSections.projects && (
            <div className="p-6 border-t border-border bg-secondary/20">
              <div className="grid grid-cols-3 gap-4">
                ...
                {projects.length === 0 && <p className="text-sm text-muted-foreground col-span-3">Aucun projet actif.</p>}
              </div>
            </div>
          )}
        </div>
        ...
        {/* LIFE BUCKETS */}
        <div className="bg-card rounded-2xl shadow-sm overflow-hidden">
          <button onClick={() => toggle("buckets")} className="flex items-center justify-between p-6 w-full hover:bg-secondary/50 transition-colors">
            <div className="flex items-center gap-3">
              {openSections.buckets ? <ChevronDown size={20} className="text-primary" /> : <ChevronRight size={20} className="text-muted-foreground" />}
              <h3 className="text-lg font-bold tracking-wide text-foreground uppercase">Life Buckets</h3>
            </div>
          ...
          {openSections.buckets && (
            <div className="p-6 border-t border-border bg-secondary/20">
              <div className="grid grid-cols-6 gap-4">
                {lifeBuckets.map(b => (
                  <div key={b.id} className="flex flex-col items-center gap-2 p-4 bg-card rounded-2xl shadow-sm border border-border">
                    <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ backgroundColor: b.color + "20" }}>
                      <span className="text-lg" style={{ color: b.color }}>●</span>
                    </div>
                    <span className="text-xs font-bold text-foreground">{b.name}</span>
                  </div>
                ))}
                {lifeBuckets.length === 0 && <p className="text-sm text-muted-foreground col-span-6">Aucun bucket.</p>}
              </div>
            </div>
          )}
        </div>
